...
const bot_name = "Devil Nexus";
const baseFolderPath = path.join("db", "past");
...
try {
  if (fs.existsSync(settingsFilePath)) {
    ...
    console.warn(
      `Settings file "${settingsFilePath}" not found. Using default settings.`
  ...
  console.error(
    `Error reading or processing settings from "${settingsFilePath}":`,
    error
  );
}
...
╭─「 DOWNLOAD MENU」`;
    ...
    default:
      await sock.sendMessage(sender, {
        text: "Invalid selection. Please reply with a number from the menu.",
      });
      break;
  }
}
...
`,
              ...
              fbMessageRequests.set(sender, { videoData, msgId: sentMessage.key.id });
          } else {
              await sock.sendMessage(sender, {
...
async function downloadUnsplashImage(sock, sender, text, messageKey) {
  ...
  if (isNaN(count) || count < 1 || count > 10) {
    await sock.sendMessage(sender, {
      text: "Please provide a valid number of images (1-10).",
    });
    return;
  }
  ...
  try {
    ...
    if (uniqueImages.length === 0) {
      await sock.sendMessage(sender, {
        text: "No unique images found. Try a different query.",
      });
      return;
    }
    ...
    await sock.sendMessage(sender, {
      text: "An error occurred while downloading images.",
    });
  }
}
...
async function handleVideoRequest(sock, sender, text, messageKey) {
  ...
  if (url && url.includes("facebook.com")) {
    try {
      const apiResponse = await axios.get(
        `https://www.dark-yasiya-api.site/download/fbdl1?url=${url}`
      );
    ...
    await sock.sendMessage(sender, {
      text: "Invalid Facebook URL format. Please provide a valid URL.",
    });
  }
}
...
async function handlefbVideoDownload(sock, sender, text, quotedMsg) {
  if (fbMessageRequests.has(sender) && (text === "1" || text === "2")) {
    ...
    const videoUrl =
      text === "1" ? videoData.videoData.hd : videoData.videoData.sd;
    const videoPath = path.resolve(
      __dirname,
      "downloads",
      `temp_fb_video_${Date.now()}.mp4`
    );
...
async function updateService(sock, sender) {
  ...
  if (!serviceName) {
    ...
    await sock.sendMessage(sender, {
      text: "*Service name not found in environment variables.*",
    });
    return;
  }
  ...
  try {
    const result = await executeCommand(
      `koyeb service update ${serviceName} --pull`
    );
    await sock.sendMessage(sender, {
    ...
    await sock.sendMessage(sender, {
      text: `*Error updating service:* ${error}`,
    });
  }
}
...
async function restartBot(sock, sender) {
  ...
  if (!serviceName) {
    console.error("Service name not found in environment variables.");
    await sock.sendMessage(sender, {
      text: "*Error:* Service name not found in environment variables.",
    });
    return;
  }
  ...
  try {
    const result = await executeCommand(
      `koyeb service redeploy ${serviceName}`
    );
    await sock.sendMessage(sender, {
    ...
    await sock.sendMessage(sender, {
      text: `*Error restarting bot:* ${error}`,
    });
  }
}
...
async function disableCommand(command, sender, sock) {
  try {
    ...
    if (settings.hasOwnProperty(command)) {
      ...
      await sock.sendMessage(sender, {
        text: `Command "${command}" has been disabled.`,
      });
    } else {
      console.log(`Command "${command}" does not exist.`);
      await sock.sendMessage(sender, {
        text: `Command "${command}" does not exist in settings.`,
      });
    }
  } catch (err) {
    console.error("Error:", err);
    await sock.sendMessage(sender, {
      text: "An error occurred while disabling the command.",
    });
  }
}
...
// Function to send the PDF list for the selected subject and year
async function sendPDFList(sock, sender, subject, year) {
  const folderPath = path.join(baseFolderPath, subject, year);
  console.log(folderPath);
  if (!fs.existsSync(folderPath)) {
    return sock.sendMessage(sender, {
      text: `No files found for this year: ${year}`,
    });
  }
  ...
  } // Notify the user // await sock.sendMessage(sender, { text: `All PDFs for the year ${year} have been sent.` });
}

// Function to send the selected PDF
async function sendSelectedPDF(sock, sender, selection, request) {
  const { subject, year, files } = request;
  const index = parseInt(selection, 10) - 1;

  if (isNaN(index) || index < 0 || index >= files.length) {
    return sock.sendMessage(sender, {
      text: "Invalid number. Please type a correct number.",
    });
  }

  const selectedPDF = files[index];
  const filePath = path.join(baseFolderPath, subject, year, selectedPDF);
...
async function startBot(sessionId) {
  ...
  sock.ev.on("messages.upsert", async (messageUpdate) => {
    ...
    const messageKey = message.key;
      ...
      const subjects = { 3: "Mathematics", 2: "Sinhala" };
      ...
      if (text === `${PREFIX}menu`) {
        ...
        if (!fbEnabled) {
          await sock.sendMessage(sender, {
            text: `*bot's owner disable ${PREFIX}fb command* 📴`,
          });
        } else {
        ...
        if (!unsplashEnabled) {
          await sock.sendMessage(sender, {
            text: `*bot's owner disable ${PREFIX}unsplash command* 📴`,
          });
        } else {
        ...
        if (!imgEnabled) {
          await sock.sendMessage(sender, {
            text: `*bot's owner disable ${PREFIX}img command* 📴`,
          });
        } else {
        ...
        await sendPDFList(sock, sender, request.subject, year);
      }else  if (text?.startsWith(`${PREFIX}tiktok`)) {
        ...
        if (
          fbMessageRequests.has(sender) &&
          fbMessageRequests.get(sender).msgId === quotedMsgId
        ) {
          await handlefbVideoDownload(sock, sender, text, quotedMsg); // Handle further processing if needed
        } else if (
          menuRequests.has(sender) &&
          menuRequests.get(sender).menuMsgId === quotedMsgId
        ) {
